...
export default class World implements IWorld {
  ...
  public update(activeActions: AllActions[], cursorPosition: Coordinates): void {
    if(this.loading) return;
    if(this.startTime === 0) this.startTime = window.performance.now(); /* set start time on first gameloop */
  ...
  private select(): void {
    switch(this.state.world.interaction.type) {
      case 'world':
        this.loading = true;
        this.eventEmitter.emit("gamePause", false);
        ...
        this.loadWoldByName(worldName).then(_result => {
          ...
          this.loading = false;
          this.eventEmitter.emit("gameStart", true);
        });
        break;
      case 'interaction': 
        break;
  ...
  private timer(): void {
    ...
    if(elapsedTimeInSeconds <= timeLimitInSeconds) {
      this.eventEmitter.emit('worldTimer', this.elapsedTime);
    } else {
