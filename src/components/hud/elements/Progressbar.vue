<script setup lang="ts">
  ...
  onMounted(() => {
    if(settings.draggable && progressbar.value) {
      progressbar.value.addEventListener('dragstart', onDragStart);
    }
  });
  onBeforeUnmount(() => {
    if(progressbar.value) {
      progressbar.value.removeEventListener('dragstart', onDragStart);
    }
  });
</script>

<template>
  <div class="element progressbar" ref="progressbar" :id="settings.name" :class="{ 'unlocked': unlocked }" :draggable="unlocked">
